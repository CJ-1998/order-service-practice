...
public class OrderController {
    ...
    public ResponseEntity<ResponseBody<Page<OrderResponseDto>>> getAllOrders(
        ...
        Page<Order> orderPage = (status != null) ? orderService.getOrdersByStatus(status, pageable) : orderService.getAllOrders(pageable);
        return ResponseEntity.ok(ResponseUtil.createSuccessResponse(orderPage.map(orderMapper::toOrderResponseDto)));
    }
